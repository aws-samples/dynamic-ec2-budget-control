...
for budget_entry in ${budgets_conf}; do
    ...
    # build slurm.conf Local Licenses line
    if [ -z "${new_entry_slurm}" ]; then
        new_entry_slurm="${_BU}:${_ADJUSTED_CORE_LIMIT}"
    else
        new_entry_slurm="${new_entry_slurm},${_BU}:${_ADJUSTED_CORE_LIMIT}"
    fi
...
# for slurm only, update slurm.conf
cp -afv "${SLURM_CONF}" "${SLURM_CONF}-${today}"  >> /var/log/budget.control.updater.log
cat "${SLURM_CONF}-${today}" | sed '/^Licenses=.*$/d' | sed '$aLicenses='${new_entry_slurm} > "${SLURM_CONF}"
